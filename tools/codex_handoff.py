...
def rule_context_lines(repo_root: pathlib.Path, paths: list[str], read_only: bool = False) -> list[str]:
    ...
    if preloaded:
        ...
        lines.extend(
            [
                "",
                "No preloaded proofd context was available. In this read-only sandbox, prefer `CLAUDE.md`, `AGENTS.md`, and already-loaded proof context over ad hoc CLI lookups.",
            ]
        )
    else:
        lines.extend(
...
def review_handoff(repo_root: pathlib.Path, args: argparse.Namespace) -> str:
    ...
    lines.extend(
        [
            "8. Do not log the run yourself. Claude will log the review after the subprocess returns.",
            "",
            "## Workspace State",
            "```text",
            status,
