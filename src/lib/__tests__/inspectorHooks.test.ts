...
// deriveStateFromPoll removed — replaced by tapStateReducer.ts (tested in tapStateReducer.test.ts)
...
// Snapshot JSON.stringify before INSTALL_HOOK can wrap it.
const _pristine = { jsonStringify: JSON.stringify };
...
function cleanupGlobalHook() {
  ...
  // Restore only JSON.stringify (wrapped by INSTALL_HOOK) — don't touch timers
  // or other globals that vi.useFakeTimers() may have wrapped.
...
// deriveStateFromPoll tests removed — replaced by tapStateReducer.test.ts
...
describe("INSTALL_HOOK stdin handler — interrupt signals", () => {
  ...
  // deriveStateFromPoll integration tests removed — legacy function deleted, replaced by tapStateReducer
});
...
// INSTALL_TAPS tests are in inspectorTaps.test.ts — separated to avoid
// OOM from INSTALL_HOOK + INSTALL_TAPS both wrapping JSON.stringify in
// the same vitest worker thread.
