...
- [LP-01 L93] At Rust startup (lib.rs run()), force WEBKIT_DISABLE_DMABUF_RENDERER=1 and WEBKIT_DISABLE_COMPOSITING_MODE=1 before tauri::Builder runs so webkit2gtk falls back to software rendering. This works around Wayland protocol Error 71 and silent hangs under XWayland on many Linux GPU/driver combinations. Pre-existing env values are honored so power users can opt back in.
...
- [PT-03 L111] CLAUDECODE env var is stripped at Rust app startup (lib.rs) so spawned PTYs do not think they are nested inside another Claude Code session.
