...
- [SP-01 L3] Slim prompt-rewrite proxy at src-tauri/src/proxy/mod.rs: forwards POST /v1/messages to https://api.anthropic.com and POST /v1/responses (or /backend-api/codex/responses) to OpenAI / chatgpt.com upstreams; applies user-defined regex rules to the provider-native prompt field (Anthropic system / OpenAI instructions) before forwarding; optionally tees request/response to per-session traffic.jsonl. Also classifies each request body as a fresh user turn vs tool-result follow-up and emits user-turn-started-{sessionId} on UserTurn so the frontend response panel clears at real send time (see AS-05). No model translation, OAuth, or compression.
