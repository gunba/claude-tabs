...
- [SI-23] Plan detection enhanced: content-based detection checks for numbered list items ('1. ' and '2. ') in assistant message text when stop reason is tool_use. actionNeeded state is preserved — subsequent ConversationMessage(assistant) events do not overwrite it. ExitPlanMode tool calls in ToolCallStart are treated as reliable state-change events (not suppressed during subagents).
  - Files: src/lib/tapStateReducer.ts, src/hooks/useTapEventProcessor.ts
