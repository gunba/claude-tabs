...
- [SI-05] Idle detection via three tap event paths: (1) TurnEnd(end_turn) in tapStateReducer (not PTY regex), (2) ConversationMessage(assistant, end_turn) reliable fallback, (3) IdlePrompt from CLI notification_type=idle_prompt — authoritative idle signal independent of SSE/streaming state. IdlePrompt bypasses subagent suppression (reliable event). `idleDetected` flag retained in INSTALL_HOOK state for legacy compatibility but not polled.
  - Files: src/lib/tapStateReducer.ts, src/lib/inspectorHooks.ts, src/lib/tapClassifier.ts, src/hooks/useTapEventProcessor.ts
...
- [SI-13] tapStateReducer event priority: actionNeeded is a sticky state preserved by an early-return guard at the top of reduceTapEvent — only UserInput, SlashCommand, UserInterruption, and PermissionPromptShown can clear it. ExitPlanMode tool call → actionNeeded; content-based plan detection (numbered list "1." + "2." in textSnippet with tool_use stop) → actionNeeded; PermissionPromptShown → waitingPermission (always wins via reduceTapBatch); UserInterruption → interrupted; TurnEnd(end_turn) → idle; TurnEnd(tool_use) → toolUse; IdlePrompt → idle (authoritative, bypasses subagent suppression). ToolCallStart(ExitPlanMode) and IdlePrompt marked reliable in useTapEventProcessor.
...
- [SI-23] Plan detection relies solely on `ToolCallStart(ExitPlanMode)` → `actionNeeded`. ToolCallStart fires during the SSE stream, structurally before any UserInput can arrive. ConversationMessage-based detection (ExitPlanMode in toolNames; content-based numbered-list heuristic) was removed: ConversationMessage fires from the stringify hook async and can arrive after UserInput has cleared actionNeeded, causing the sticky guard to re-trap the state while the agent is actively working. ToolCallStart(ExitPlanMode) is NOT in the reliable-events list — if subagents are active it is suppressed (subagent ExitPlanMode calls must not affect the main session state). actionNeeded is sticky: only UserInput, SlashCommand, UserInterruption, PermissionPromptShown can clear it.
  - Files: src/lib/tapStateReducer.ts, src/hooks/useTapEventProcessor.ts
- [SI-24] Tab flash debounce (2s): flash notifications for idle state changes are debounced with a 2-second timer. Pending flashes are cancelled when the session goes non-idle again, preventing false alerts during transient idle windows during active tool use.
  - Files: src/App.tsx
- [SI-25] Status line data capture: INSTALL_TAPS stringify hook detects hook_event_name==='Status' (Claude Code's status line JSON payload) and pushes flattened fields to dedicated 'status-line' category. tapClassifier classifies as StatusLineUpdate event (28 fields: sessionId, cliVersion, modelId, fiveHourUsedPercent, sevenDayUsedPercent, contextUsedPercent, vimMode, etc.). tapMetadataAccumulator stores as grouped nullable statusLine object on SessionMetadata (19 fields, excludes sessionId/cwd/modelId/modelDisplayName which are session-level). tapStateReducer treats as informational (no state change). Dual push: both status-line and generic stringify entries fire — stringify truncated snap harmlessly classifies as null. Data available via session.metadata.statusLine for future UI (tab headers, status bar). No UI exposure yet.
  - Files: src/lib/inspectorHooks.ts, src/lib/tapClassifier.ts, src/lib/tapMetadataAccumulator.ts, src/lib/tapStateReducer.ts, src/types/tapEvents.ts, src/types/session.ts
