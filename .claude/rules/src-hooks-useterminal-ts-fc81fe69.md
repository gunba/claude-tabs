...
- [TA-10 L373] Auto-rename from OSC 0: term.onTitleChange fires when Claude Code's Haiku subagent emits a title via OSC 0 (Linux/macOS). Handler strips leading non-letter/digit chars (spinners, bullets) via Unicode property escape regex, then skips if title is empty, equals 'Code Tabs', starts with 'Claude Code', or equals 'claude' (case-insensitive) — the last guard prevents a bare 'claude' resume title from clobbering a Haiku-generated session name. On valid title, calls renameSession(sid, title) and setSessionName(getResumeId(session), title). Windows uses process.title and does not fire OSC 0.
