...
- [PM-02 L66] spawn_collector(app) runs a dedicated background thread polling sysinfo every 1000ms. System initialized with RefreshKind::nothing().with_cpu(CpuRefreshKind::nothing()).with_processes(...), so cpus().len() returns the logical CPU count immediately without a System::new_all() fallback. CPU% is normalized by cpu_count to match Task Manager / Activity Monitor semantics (100% = one full core).
