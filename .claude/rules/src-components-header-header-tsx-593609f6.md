...
- [BR-01 L60] Code Tabs branding is applied across tauri.conf.json productName and window title, index.html <title>, src-tauri/src/lib.rs build expect string, Header label, and App.tsx dynamic window title (format: 'Code Tabs [vX.Y.Z] · Claude X · Codex Y'). App.tsx window title includes both cliVersions.claude and cliVersions.codex ('not installed' when absent).
