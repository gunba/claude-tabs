...
- [PT-19 L153] TERM=xterm-ghostty, TERM_PROGRAM=ghostty, and COLORTERM=truecolor are injected before the caller-supplied env in unix.rs spawn(), so caller entries win on conflict. This ensures color-aware CLIs get a capable terminal type and enables TUI sync output (see PT-23 for the ghostty-specific DEC 2026 rationale).
  - src-tauri/src/pty/unix.rs:L206
- [PT-23 L154] Linux PTY spawn sets TERM=xterm-ghostty + TERM_PROGRAM=ghostty before the caller env so Claude Code's isSynchronizedOutputSupported() (env-sniff on TERM / TERM_PROGRAM in src/ink/terminal.ts) returns true and the TUI wraps diff frames in BSU/ESU (DEC 2026). Without this, ink.tsx L736 passes skipSyncMarkers=true and emits raw incremental patches; each keystroke's render output lands one-behind in xterm.js because the buffer only flushes when the next input triggers another render. xterm.js 6.0 handles DEC 2026 correctly; ghostty's terminfo is xterm-compatible so color/mouse/key sequences stay identical.
- [PT-18 L495] Shutdown drain: pty_drain_output command empties the channel (500ms deadline, 10ms intervals) before session destroy, preventing the background reader thread from blocking on a full channel.
