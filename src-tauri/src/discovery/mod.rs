//! Discovery functions for Claude Code CLI.
//!
//! These are the pure sync primitives used by both the runtime Tauri commands
//! (see `commands::cli`) and the standalone `discover_audit` binary
//! (`bin/discover_audit.rs`). Keeping them here, behind no Tauri types, ensures
//! the audit tool runs the exact same code as the app — one pipeline, one truth.
//!
// [DM-01] Discovery module: pure sync primitives shared by runtime app and discover_audit binary
...
/// Callers (the Tauri wrapper) use this to log a debug warning so silent drops
/// stop being silent.
#[derive(Debug, Clone)]
...
pub(crate) fn scan_skill_md(
    ...
    // Split frontmatter (if present) from the body.
    let (fm_name, fm_desc, body) = parse_skill_frontmatter(&content);

    // Name: frontmatter > parent directory name. Empty parent -> reject.
    ...
    // Description: frontmatter > first non-empty body line (truncated).
    let desc = fm_desc
        .or_else(|| {
            body.lines()
                .map(|l| l.trim())
        ...
        .map(|s| {
            // Strip surrounding matching quotes (YAML may emit "..." or '...').
            let s = s.trim();
            let s = s
                .strip_prefix('"')
...
/// Extract the name, description, and body text from a SKILL.md.
/// Returns (frontmatter_name, frontmatter_description, body) where each may be absent.
fn parse_skill_frontmatter(content: &str) -> (Option<String>, Option<String>, &str) {
    // Accept a leading BOM or whitespace but require `---\n` at the start of frontmatter.
    ...
    let end = match rest.find("\n---") {
        Some(i) => i,
        // Malformed frontmatter (no closing ---). Treat the rest as body.
        None => return (None, None, trimmed),
    };
    let meta = &rest[..end];
    let body_start = end + "\n---".len();
...
/// Matches what Claude Code accepts: starts with a letter, alphanumeric + `-_`.
...
mod tests {
    ...
    fn frontmatter_malformed_treated_as_body() {
        let input = "---\nname: x\nno closing dashes";
        let (name, desc, _body) = parse_skill_frontmatter(input);
        // No closing ---, so whole thing is body.
        assert!(name.is_none());
        assert!(desc.is_none());
    }
