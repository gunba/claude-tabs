...
6. **De-Claude-ify rules files.** `.claude/rules/*.md` files reference Claude-only behavior in tag lines; the externally-generated regeneration pass needs to either widen to both CLIs or split into `claude/` and `codex/` subdirs.
